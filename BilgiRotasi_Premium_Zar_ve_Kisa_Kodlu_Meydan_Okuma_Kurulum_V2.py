...
required_files = [MAIN, GAME_UI, NAV, SOCIAL, ABOUT, PUBSPEC, TEST]
...
intended_paths = [str(path) for path in required_files]
intended_status = subprocess.run(
    ["git", "status", "--porcelain", "--", *intended_paths],
    text=True,
    capture_output=True,
    check=True,
).stdout.strip()
...
if intended_status:
    raise SystemExit(
        "Kurulumun değiştireceği dosyalarda önceden yerel değişiklik var:\n"
        f"{intended_status}\n"
        "Bu dosyaları önce commit et veya değişikliklerini güvenli bir dalda sakla."
    )

unrelated_status = subprocess.run(
...
ignored_names = {
    "BilgiRotasi_Premium_Zar_ve_Kisa_Kodlu_Meydan_Okuma_Kurulum.py",
    "BilgiRotasi_Premium_Zar_ve_Kisa_Kodlu_Meydan_Okuma_Kurulum_V2.py",
}
visible_unrelated = [
    line for line in unrelated_status
    if not any(name in line for name in ignored_names)
]
...
if version != (1, 41, 2, 53):
    raise SystemExit(
        "Bu paket 1.41.2+53 sürümü için hazırlandı.\n"
...
\s+emoji: '👨‍👩‍👧‍👦')""",
    ...
    about = require_replace(
        about,
        "Sürüm 1.41.2+53",
        "Sürüm 1.42.0+54",
        "Hakkında ekranı sürüm bilgisi",
    )
...
"""
    ...
    checks = {
        ...
        PUBSPEC: ["version: 1.42.0+54"],
        ABOUT: ["Sürüm 1.42.0+54"],
        TEST: [
    ...
    validation_paths = [
        *intended_paths,
        str(DICE_TARGET),
        str(CHALLENGE_TARGET),
    ]
    run(["git", "diff", "--check", "--", *validation_paths])
